...
def display_semantic_interface(lang_code, nlp_models, t):
                    ...
                    with st.spinner("Analyzing..."):
                        try:
                            ...
                            if 'graph_id' not in st.session_state:
                                ...
                                st.write(f"New graph created with ID: {st.session_state.graph_id}")
                            else:
                                update_float_content(st.session_state.graph_id, graph_content)
                                st.write(f"Existing graph updated with ID: {st.session_state.graph_id}")
                            ...
                            st.write(f"Debug: Graph visible: {st.session_state.graph_visible}")
    ...
    # Añadir botones para controlar el elemento flotante
    ...
    with col1:
        if st.button("Show Graph"):
            if 'graph_id' in st.session_state:
                toggle_float_visibility(st.session_state.graph_id, True)
                st.session_state.graph_visible = True
                st.write(f"Showing graph with ID: {st.session_state.graph_id}")
            else:
                st.write("No graph available to show")
    ...
    with col2:
        if st.button("Hide Graph"):
            if 'graph_id' in st.session_state:
                ...
                st.session_state.graph_visible = False
                st.write(f"Hiding graph with ID: {st.session_state.graph_id}")
            else:
                st.write("No graph available to hide")