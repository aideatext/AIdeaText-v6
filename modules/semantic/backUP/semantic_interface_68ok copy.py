...
from .semantic_float68ok import *
            ...
            if file_contents:
                with st.spinner("Analyzing..."):
                    try:
                        ...
                        if concept_graph_base64:
                            graph_content = f"""
                                <h3>Key Concepts:</h3>
                                <p>{', '.join([f"{concept}: {freq:.2f}" for concept, freq in key_concepts])}</p>
                                <img src="data:image/png;base64,{concept_graph_base64}" alt="Concept Graph" style="width:100%; height:auto;"/>
                            """
                            st.session_state.graph_id = float_graph(graph_content, width="800px", height="600px", position="center-right")
                            st.session_state.graph_visible = True
                            st.session_state.graph_content = graph_content

                        if entity_graph_base64:
                            entity_graph_content = f"""
                                <h3>Entity Graph:</h3>
                                <img src="data:image/png;base64,{entity_graph_base64}" alt="Entity Graph" style="width:100%; height:auto;"/>
                            """
                            st.session_state.entity_graph_id = float_graph(entity_graph_content, width="800px", height="600px", position="bottom-left")

                        # Log para depuración
                        st.write(f"Debug: Graph ID: {st.session_state.get('graph_id')}")
                        st.write(f"Debug: Graph visible: {st.session_state.get('graph_visible')}")
                        st.write(f"Debug: Concept graph base64 (first 100 chars): {concept_graph_base64[:100]}")
...
if st.session_state.get('graph_visible', False) and 'graph_content' in st.session_state:
    components.html(
        f"""
        <div id="{st.session_state.graph_id}" class="semantic-float" style="display:block;">
            {st.session_state.graph_content}
        </div>
        """,
        height=600,
