...
def display_semantic_interface(lang_code, nlp_models, t):
                    ...
                    with st.spinner("Analyzing..."):
                        try:
                            ...
                            if 'graph_id' not in st.session_state:
                                st.session_state.graph_id = float_graph(graph_content, width="40%", height="60%", position="top-right")
                            else:
    ...
    # Botón para alternar la visibilidad del grafo flotante
    if 'graph_id' in st.session_state:
        if st.button("Toggle Graph Visibility"):
            toggle_float_visibility(st.session_state.graph_id, not st.session_state.get('graph_visible', True))
